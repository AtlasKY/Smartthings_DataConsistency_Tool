...
//Class that reads the AST and extracts the information the analysis class will need
//stores the information in Handler objects in a list of handlers
...
class CTAnalysisAST extends CompilationCustomizer{	
	...
	void getStateVariables(ClassNode cn) {
		...
		//s: Scheduler handler
		//m: method that the recursion is currently in to prevent recursive infinite loops in analysis
		
		//the path logs a trace of situations outlined above that affects the analysis of the current line of code
		//as the situations are encountered going down the AST, they are concatenated to the end of the path
		//when going up the AST, the flags down the AST are removed
		//one can think of it as a stack-like implementation
		def pathLog
		...
		handlers.each { hdl->
			...
			//a list to prevent concurrent modification error in the loop below
			//store the method nodes to be iterated over
			List methTemps = new ArrayList()
			...
			hdl.calledMethods.each { mt->
				...
				//then add the method to the temp list
				//to get the method node and call the method node handler
	...
	void stateRecurse(Statement st, Handler hdl, ClassNode cn, String pth) {
		...
		if(st instanceof BlockStatement) {
			st.getStatements().each { bst->
				...
				//check if the range of the for loop in for(locVar in range) format
				//corresponds to a list of devices
				//if it is then pass it into the path variable
				...
				//iterate over the statements inside the for loop block
		...
		//if it is an if statement call this method on the boolean expression, if block, and else block
			...
			//a string to log the conditional flags
			def mods = ""
			//get the boolean conditional of the if statement
			Statement est = new ExpressionStatement((Expression)st.getBooleanExpression())
			
			//recurse over the boolean exp
			stateRecurse(est, hdl, cn, pth + "b:")//boolean block
			
			//if there is an else block, log el-, if there's no else block log i-
			...
			//if the conditional uses time info like now() or time keyword
			//then log t- to the flags
			if((est.getText().contains("now") || est.getText().contains("time")) &&
				 !(est.getText().contains("log"))) {
				mods += "t-"
			}
			
			//if the boolean check uses state info,
			//log s- to the flags
			if(est.getText().toLowerCase().contains("state.")) {
				mods += "s-"
			}
			
			//check for event information usage in the conditional check
			//iterate over the arguments to the handler, i.e. evt variable
			//if the boolean contains the argument, then it uses event information
			//log e- + argument.event value accessed + |- 
			hdl.args.each { a-> 
			...
			//if the boolean exp is a method call, get state related information
			...
			//recurse over the if and else blocks with respective path flags
		...
		//cast it as expression and analyze
			...
			if(exp instanceof MethodCallExpression) {
				...
				if(!pth.contains(mname)) {
					...
					//check if the method is an unschedule method
					boolean unSch = false
					...
					//if expression uses time info, log the time access in the handler
					...
					if(mtext.equals("runin") || (mtext.equals("schedule") && !mtext.equals("unschedule"))
						...
						//check for whether it contains overwrite flag and if it set to false or not
						...
						//Pass the appropriate flag depending on whether the schedule overwrite is true or not
						if(schOvrWr) {
							hdl.setSch(schOvrWr)
					...
					//Goes into inside of them to check for device related calls
						...
						//store the variable used within the .each loop as parName
						def parName = ""
			...
			//if the expression is a binary expression
				...
				if(lex.text.toLowerCase().contains("state.") && !pth.contains("br:")) {
				...
				if(rex.text.toLowerCase().contains("state.")) {
					if(exp.getOperation().getText().contains("==") || exp.getOperation().getText().contains("<")
						|| exp.getOperation().getText().contains(">") || exp.getOperation().getText().contains("!")) {
			...
			//if boolean expression, 
				...
				if(exp.getText().toLowerCase().contains("state.") && !pth.contains("bl:")) {
...
//								print "Device Cap: " 
					...
					//if it is a device add to devices list
					if(isDevice)
			...
			if(mceText.equals("subscribe") && mce.getArguments().size()>0) {
				...
				//to be called handler name, subscribed device name, and subscribed event name
				def hname = ""
				def dname = ""
				def ename = ""
				//if has equal to or more than 3 arguments
				if(arglist.size() >= 3) {
					
					//first argument is the device to subscribe to
					...
					//second argument is the capabilities or even triggers to subscribe to
					...
					//last is the handler to be called when the event triggers
			...
			//if it is a schedule method, then create a different kind of handler object
